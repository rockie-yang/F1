with
    driver_points as (
        ...
        from {{ ref("stg_f1_dataset__drivers") }} as drivers
        join
            {{ ref("stg_f1_dataset__results") }} as results
            on drivers.driver_id = results.driver_id
        join {{ ref("stg_f1_dataset__races") }} races on results.race_id = races.race_id
        join
            {{ ref("stg_f1_dataset__driver_standings") }} as driver_standings
            on driver_standings.race_id = races.race_id
